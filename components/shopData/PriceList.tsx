    ...
    <div className="w-full bg-white p-5">
      ...
      <RadioGroup className="mt-2 space-y-1" value={selectedPrice || ""}>
        {priceArray?.map((price, index) => (
          ...
          >
            ...
            <Label
              htmlFor={price.value}
              className={`${selectedPrice === price?.value ? "font-semibold text-[#063c28]" : "font-normal"}`}
            >
              {price?.title}
            </Label>
          </div>
      ...
      {selectedPrice && (
        <button
          ...
          className="text-sm font-medium mt-2 underline underline-offset-2 decoration-[1px] hover:text-[#063c28] hoverEffect"
        >
          Reset selection
        </button>
      )}
